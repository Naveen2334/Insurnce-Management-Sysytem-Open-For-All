...
<body>
	...
	<div class="content">
		...
		if (custList == null) {
		%>
		<form action="CustomerController" method="post" name="viewform"
			class="basic-grey" onsubmit="return dateMandatory()">
			<h1>View Customers</h1>
			<label> <span>Start Date :<sup><font color="red">*</font></sup></span>
				<input type="date" name="sdate" onblur="startdate(this.value)" />
				<p id="sdate_msg">
					<%
					if (request.getAttribute("sdate") != null && request.getAttribute("sdate").toString().equals("sdate")) {
						out.print("Start Date must not be empty.");
					}
					%>
				</p>
			</label> <label> <span>End Date :<sup><font color="red">*</font></sup></span>
				<input type="date" name="edate" onblur="enddate(this.value)" />
				<p id="edate_msg">
					<%
					if (request.getAttribute("edate") != null && request.getAttribute("edate").toString().equals("edate")) {
						out.print("End Date must not be empty.");
					}
					%>
				</p>
			</label>
			...
			<br> <input type="hidden" name="action" value="viewCustomer">
