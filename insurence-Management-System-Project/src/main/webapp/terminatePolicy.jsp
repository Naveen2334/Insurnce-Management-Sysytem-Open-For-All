...
<%@ page
	import="com.jsp.insurence_management.dto.*, java.util.ArrayList"%>
<%
...
<body>
	...
	<div class="content">
		...
		<form action="InsuranceController" method="post"
			name="removepolicyform" class="basic-grey"
			onsubmit="return remove_policy_mandatory()">
			<h1>Remove Policy</h1>

			<label> <span>Customer ID :<sup><font
						color="red">*</font></sup></span> <input type="text" name="custid" id="custid"
				maxlength="6" onblur="CustId(this.value)" />
				<p id="custid_msg">
					<%
					if (request.getAttribute("custid") != null && request.getAttribute("custid").toString().equals("custid")) {
						out.print("Customer id  must be in NUMBER only and not empty.");
					}
					%>
				</p>
			</label>
			...
			<center>
				<span>&nbsp;</span> <input type="submit" value="View" /><input
					type="reset" value="reset" /> <input type="hidden" name="action"
					value="viewPolicies" /> <input type="hidden" name="check"
					value="remove" />
			</center>
...
if (ter2 > 0) {%><%=notTer%> policy not terminated.<%}%>
		...
		<form name="Terminatepolicyform" action="InsuranceController"
			method="post" onsubmit="return checkbox1()">
			<table align="center" border="1" cellspacing="5" cellpadding="5">
				...
				<tr>
					...
					<td><input type="checkbox" name="select"
						<%if (insurance.getPolicyStatus().equalsIgnoreCase("Active")) {%>
						value="<%=insurance.getPolicyReferenceNumber()%>"
						<%status = 1;
} else {%> disabled <%}%> /></td>
					...
					<%
					if (insurance.getVehicleNo() == null || insurance.getVehicleNo() == "") {
					%>
					<td>NA</td>
					<%
					} else {
					...
					if (insurance.getLicenseNo() == null || insurance.getLicenseNo() == "") {
					%>
					<td>NA</td>
					<%
					} else {
					...
					if (insurance.getVehicleType() == null || insurance.getVehicleType() == "") {
					%>
					<td>NA</td>
					<%
					} else {
			...
			%>
			<input type="hidden" name="action" value="terminate" /> <input
				type="button" value="Back" onClick="window.history.back()" />
		</form>
		<%
		} else {
		%>
