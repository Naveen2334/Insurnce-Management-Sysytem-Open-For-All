...
<body>
	...
	<div class="display">
		...
		<p>
			...
			%>
			Policy registered successfully.<br>Policy reference number is
			<%=request.getAttribute("check")%>.<br>Policy end date is
			<%=request.getAttribute("end_date").toString()%>.
			<%
			...
			%>
			Customer registered successfully.<br>Customer id is
			<%=request.getAttribute("check")%>.
			<%
			} else if ("applyclaim".equalsIgnoreCase(confirm)) {
			%>
			Claim registered successfully.<br>Claim id is
			<%=request.getAttribute("check")%>.<%
			}
			%>
		</p>
